...
describe('audit-export.test.js', () => {
  it('runs assertions', () => {
    ...
    var rows = AuditExport.buildMakeupLogRows(sem);
    ...
    assert(AuditExport.suggestedPdfName(sem, 2) === 'Fall-2026-REGN15P-Audit-v2.pdf',
    ...
    var migrated = DataModel.migrateSemester(JSON.parse(JSON.stringify(legacy)));
    ...
    var unfinalized = DataModel.migrateSemester({
      id: 'legacy2',
    ...
    ['REGN15P', 'REGN25P', 'REGN35P-36P', 'REGN48P'].forEach(function (courseId) {
      var course = CourseDefaults.get(courseId);
      assert(course && course.courseId === courseId, 'course defaults exist for ' + courseId);
      var target = DataModel.createDefaultSemester();
      CourseDefaults.applyToSemester(target, courseId);
    ...
    assert(Audit.getPhase(gated) === 'setup', 'new semester starts in setup');
    assert(Audit.canEdit(gated, 'setup'), 'setup phase is editable');
    assert(Audit.transitionError(gated, 'active') !== null,
      'cannot open semester without lead faculty');
    gated.meta.leadFaculty.name = 'Lead Faculty';
    assert(Audit.transitionError(gated, 'active') === null, 'open allowed with lead faculty');
    assert(Audit.setPhase(gated, 'active'), 'setup -> active');
    assert(Audit.setPhase(gated, 'locked') === false, 'active -> locked rejected');
    assert(Audit.setPhase(gated, 'makeup_review'), 'active -> makeup_review');
    assert(Audit.canEdit(gated, 'makeup'), 'makeup_review still editable');
    assert(Audit.transitionError(gated, 'audit_exported') !== null,
    ...
    assert(Audit.setPhase(gated, 'audit_exported'), 'attested review -> audit_exported');
    assert(!Audit.canEdit(gated, 'setup'), 'audit_exported blocks setup edits');
    assert(!Audit.canEdit(gated, 'makeup'), 'audit_exported blocks makeup edits');
    assert(!Audit.canEdit(gated, 'masterCell'), 'audit_exported blocks master cell edits');
    assert(Audit.transitionError(gated, 'locked') !== null, 'lock blocked before export recorded');
    ...
    assert(Audit.setPhase(gated, 'locked', { actorName: 'Admin' }), 'exported -> locked');
    assert(Audit.isLocked(gated), 'locked semester reports isLocked');
    assert(gated.meta.lock.lockedByName === 'Admin', 'lock records actor');
    assert(Audit.setPhase(gated, 'active') === false, 'locked is terminal');
