...
import { DataModel, RosterBalance } from './_harness.js';

describe('roster-balance.test.js', () => {
  it('runs assertions', () => {
    let failed = 0;
    ...
    function makeStudents(total) {
      ...
      for (var i = 0; i < total; i++) {
        students.push(DataModel.createStudent('Student ' + (i + 1), 'C1', 'SG1', 'fac1', ''));
      }
      return students;
    }
    ...
    function assertMatchesMap(students, config, label) {
      var map = RosterBalance.buildClinicalToSimMap(
    ...
    (function testStandardFiveGroups() {
      ...
      config = DataModel.normalizeConfig(config);
      var students = makeStudents(30);
      RosterBalance.rebalance(students, config);
      assertClinicalBalance(students, config.clinicalGroups, 6, label);
      assertSimCohortConsistency(students, config.clinicalGroups, label);
      assertMatchesMap(students, config, label);
      var map = RosterBalance.buildClinicalToSimMap(config.clinicalGroups, config.simGroups);
