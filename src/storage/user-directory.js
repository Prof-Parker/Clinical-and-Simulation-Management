...
import * as UsersRegistryStorage from './users-registry-storage.js';

var LEAD_FACULTY_LIMIT = 8;

  function registryUsers() {
    if (!UsersRegistryStorage || !UsersRegistryStorage.isReady()) return null;
  ...
  function toUserRecord(userId, entry) {
    var displayName = UserData.formatFullName(entry.firstName, entry.lastName);
