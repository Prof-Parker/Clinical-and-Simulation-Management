...
function clinicalFacilitySelectHtml(data, student, weekIndex, cell) {
    ...
    var selected = (cell && cell.facilityId) ||
      (ClinicalSites
        ? ClinicalSites.resolveFacilityForWeek(data, student.clinicalGroup, weekIndex, 0)
        : student.facilityId);
  ...
  function openCellEditor(studentId, weekIndex) {
    ...
    var groupOrient = Orientation && Orientation.getGroupOrientation(data, student.clinicalGroup);
    ...
    showDialog('Edit Schedule Cell', html, function () {
      var orientOn = document.getElementById('editOrientation').checked;
      if (orientOn) {
        student.orientationWeekIndex = weekIndex;
      } else if (Orientation && Orientation.isOrientationWeek(data, student, weekIndex)) {
