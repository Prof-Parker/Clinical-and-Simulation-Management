...
import {
  studentNameInputsHtml,
  studentEmailInputHtml,
  sortStudentsWithinCohorts
} from './roster-names.js';
import {
  predominantSection,
...
function createNewStudentForGroup(data, clinicalGroup) {
  ...
  return DataModel.createStudent(
...
function renderRosterHeaderExtras(data) {
  var showDomainEl = setupEl('showStudentEmailDomain');
  if (showDomainEl) {
    showDomainEl.checked = !(data.meta && data.meta.showStudentEmailDomain === false);
  }
}

function renderRoster(data) {
  renderRosterHeaderExtras(data);
  ...
  var columnHeadersHtml =
    ...
    '<span></span><span>Move</span><span>Last | First</span><span>Email</span><span>Section</span><span>Sim</span><span>Site</span><span></span>' +
    '</div>';
...
function sortRosterAlphabetically(data) {
  collectFromForm(data);
  var groups = DataModel.getClinicalGroups(data.config);
  data.students = sortStudentsWithinCohorts(data.students, groups);
...
function initOneRoster(roster) {
  ...
  roster.addEventListener('change', function (e) {
    ...
    if (e.target.hasAttribute('data-cohort-section-bulk')) {
      ...
      if (!guardSetupEdit()) return;
      var dataSec = resolveSetupData();
      collectFromForm(dataSec);
      applyCohortSection(dataSec, e.target.getAttribute('data-cohort-section-bulk'), sectionName);
      setupAfterChange(dataSec);
      return;
    }
    if (e.target.hasAttribute('data-cohort-sim-bulk')) {
      var simGroup = e.target.value;
      if (!simGroup) return;
      if (!guardSetupEdit()) return;
      var dataSim = resolveSetupData();
      collectFromForm(dataSim);
      applyCohortSimGroup(dataSim, e.target.getAttribute('data-cohort-sim-bulk'), simGroup);
      setupAfterChange(dataSim);
      return;
    }
    if (e.target.classList.contains('move-cohort-select')) {
      ...
      if (!target) return;
      if (!guardSetupEdit()) return;
      var dataMove = resolveSetupData();
      collectFromForm(dataMove);
      moveStudentToGroup(dataMove, e.target.getAttribute('data-student-id'), target);
    }
  });
