...
var filterState = {
  ...
  function render() {
    ...
    document.getElementById('usersCreateBtn').addEventListener('click', function () {
      ...
      sel.innerHTML = UserTemplate.listRoles().map(function (r) {
  ...
  function createUser() {
    ...
    var key = UserData.generateKey();
    UserData.hashKey(key).then(function (keyHash) {
      var session = UserSession.getSession();
      var profile = { firstName: firstName, lastName: lastName, email: email };
      var entry = UserData.createRegistryEntry(role, keyHash, session ? session.name : '', profile);
      UsersRegistryStorage.addOrUpdateUser(userId, entry);
      var userFile = UserData.createUserFile(userId, firstName, lastName, email, key);
      var filename = (firstName + '-' + lastName).replace(/\s+/g, '-').toLowerCase() + '.user.json';
      UserStorage.exportUserFileDownload(userFile, filename);
      UsersRegistryStorage.mergeSave(UsersRegistryStorage.getRegistry());
      showAlert('User created', 'Downloaded user file for ' + fullName + '. Share read-only with the user.');
      render();
      if (refresh) refresh();
    });
  }
  ...
  function reissueKey(userId) {
    ...
    UserData.hashKey(key).then(function (keyHash) {
      ...
      showAlert('Key reissued', 'Update the user file manually or create a new download from registry.');
      render();
    });
  }
