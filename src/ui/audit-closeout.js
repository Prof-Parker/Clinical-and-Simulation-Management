/**
 * Audit closeout workflow tab.
 */

import * as Audit from '../audit/audit.js';
import * as AuditExport from '../audit/audit-export.js';
...
var groupFilter = '';
  ...
  function makeupTableHtml(data) {
    ...
    var body = rows.map(function (r) {
      var tier = MakeupDisplay.getSlotTier(r.makeup);
      return '<tr class="' + MakeupDisplay.tierClass(tier) + '">' +
  ...
  function adminControlsHtml(data) {
    var phase = Audit.getPhase(data);
    var attested = Audit.isAttested(data);
    var buttons = [];
    if (phase === 'setup') {
  ...
  function transitionWithConfirm(nextPhase, title, message, options) {
    var data = getData();
    var err = Audit.transitionError(data, nextPhase);
    if (err) {
      showAlert('Cannot continue', err);
      return;
    }
    showConfirm(title, message, function () {
      Audit.setPhase(data, nextPhase, options || {});
      notifyChange();
      render(data);
  ...
  function submitAttestation() {
    ...
    if (!check || !check.checked) {
      showAlert('Attestation', 'Check the attestation statement to continue.');
      return;
    }
    if (!name || !name.value.trim()) {
      showAlert('Attestation', 'Enter the attesting faculty name.');
      return;
    }
    ...
    notifyChange();
    render(data);
    showAlert('Attestation recorded',
  ...
  function handleExportPdf() {
    ...
    if (!AuditExport) {
      showAlert('Unavailable', 'Audit PDF export module not loaded.');
      return;
    }
    AuditExport.promptAndExport(data, function () {
      render(getData());
    });
  }
