/**
 * Orientation week detection and labels.
 */
...
var KNOWN_INITIALS = {
  ...
  function getGroupOrientation(data, clinicalGroup, facilityId) {
    ...
    if (facilityId) {
      return list.find(function (o) {
        return DataModel.sameFacilitySite(data, o.facilityId, facilityId);
      }) || null;
    }
    return list[0];
  }
  ...
  function getOrientationForWeek(data, student, weekIndex) {
    ...
    for (var i = 0; i < list.length; i++) {
      ...
      if (CalendarEngine.getWeekIndexForDate(data, o.date) === weekIndex) return o;
    }
    return null;
  }
  ...
  function getOrientationWeeksForStudent(data, student) {
    ...
    getGroupOrientations(data, student.clinicalGroup).forEach(function (o) {
      ...
      var wi = CalendarEngine.getWeekIndexForDate(data, o.date);
...
export {
  facilityInitials,
  getGroupOrientations,
  getGroupOrientation,
  getOrientationForWeek,
  getOrientationFacilityId,
  getOrientationWeeksForStudent,
  getEffectiveOrientationWeekIndex,
  isOrientationWeek,
  getOrientationLabel,
  orientationLabelForExport,
  weekHasOrientationConflict,
  findOrientationConflicts
};
