...
function emptyAdjustments() {
  ...
  function getSimCalendar(data) {
    ...
    return Scheduler.buildProgramSimCalendar(data, data.config);
  }
  ...
  function isSubstitutionPlacement(student, cell, weekIndex, calendar, simGroups, data) {
    ...
    var primary = Scheduler.getStudentSimSlot(student, cell.sim, calendar, simGroups, data);
  ...
  function collectIncompleteStudents(data) {
    ...
    if (Validator.validateStudentSimParticipation) {
      Validator.validateStudentSimParticipation(data).forEach(function (v) {
  ...
  function informationalNotes(data) {
    if (!Feasibility || !Feasibility.checkInformational) return [];
    return Feasibility.checkInformational(data).issues.map(function (issue) {
      return Feasibility.formatIssue(issue);
    });
  }
  ...
  function summarize(data) {
    ...
    result.blockingIssues = Feasibility.checkBlocking(data).issues;
    result.notes = informationalNotes(data);
    if (ClinicalSites.getSiteWeekPlanNotes) {
      result.notes = result.notes.concat(ClinicalSites.getSiteWeekPlanNotes(data));
    }
    ...
    if (result.generated) {
      ...
      result.orientationConflicts = Orientation
        ? Orientation.findOrientationConflicts(data)
        : [];
  ...
  function formatBlockingIssue(issue) {
    if (Feasibility.formatIssue) {
      return Feasibility.formatIssue(issue, { blocking: true });
    }
