/**
 * Clinical site assignment per group/week.
 */
...
function getGroupFacilities(data, group) {
    ...
    var cfg = DataModel.normalizeConfig(data.config);
    ...
    if (!raw.length && data.facilities && data.facilities.length) {
      var def = DataModel.getDefaultFacilityIdForClinicalGroup(group, data.facilities);
      return def ? [DataModel.getCanonicalFacilityId(data, def)] : [];
    }
    var seen = {};
    var list = [];
    raw.forEach(function (id) {
      if (!id) return;
      var canon = DataModel.getCanonicalFacilityId(data, id);
      if (!canon || !DataModel.findFacilityById(data, canon)) return;
      var key = canon;
      if (seen[key]) return;
      seen[key] = true;
      list.push(canon);
  ...
  function normalizeSiteWeekRange(data, group, range) {
    ...
    var facId = DataModel.getCanonicalFacilityId(data, range.facilityId);
    if (!facId || !DataModel.findFacilityById(data, facId)) return null;
    var ok = allowed.some(function (id) {
      return DataModel.sameFacilitySite(data, id, facId);
    });
    if (!ok) return null;
  ...
  function getStudentFacilityAtWeek(data, student, weekIndex) {
    ...
    if (student.facilityId) {
      return DataModel.getCanonicalFacilityId(data, student.facilityId);
    }
    return null;
  }
  ...
  function studentAtFacilityAtWeek(data, student, weekIndex, facilityId) {
    ...
    return DataModel.sameFacilitySite(data, atWeek, facilityId);
  }
  ...
  function facilityInitialsForCell(data, student, weekIndex) {
    ...
    if (!facId) return '';
    if (Orientation.facilityInitials) {
  ...
  function studentHasAnyWeekAtFacility(data, student, facilityId) {
    ...
    return DataModel.sameFacilitySite(data, student.facilityId, facilityId);
  }
  ...
  function validateGroupSiteWeeks(data, group) {
    ...
    raw.forEach(function (r, idx) {
      ...
      var facId = DataModel.getCanonicalFacilityId(data, r.facilityId);
  ...
  function isWeekGapForGroup(data, group, weekIndex) {
    ...
    if (CalendarEngine.isWeekInactive(data, weekIndex)) return false;
    var clinStart = (data.config.clinicalStartWeek || 5) - 1;
  ...
  function getSiteWeekPlanNotes(data) {
    ...
    DataModel.getClinicalGroups(data.config).forEach(function (group) {
      if (!groupUsesWeekRanges(data, group)) return;
  ...
  function collectGroupFacilitiesFromDom(data) {
    ...
    groups.forEach(function (g) {
      ...
      if (!cfg.clinicalGroupFacilities[g].length) {
        var fallback = DataModel.getDefaultFacilityIdForClinicalGroup(g, data.facilities || []);
...
export {
  getGroupFacilities,
  getPrimaryGroupFacility,
  groupHasMultipleSites,
  groupUsesWeekRanges,
  getGroupSiteWeekRanges,
  resolveFacilityForClinicalOrdinal,
  resolveFacilityForWeek,
  getStudentFacilityAtWeek,
  studentAtFacilityAtWeek,
  facilityInitialsForCell,
  studentHasAnyWeekAtFacility,
  applyPrimarySitesToStudents,
  collectGroupFacilitiesFromDom,
  collectGroupSiteWeeksFromDom,
  normalizeGroupSiteWeeks,
  validateGroupSiteWeeks,
  findGroupSiteWeekGaps,
  isWeekGapForGroup,
  getSiteWeekPlanNotes
};
