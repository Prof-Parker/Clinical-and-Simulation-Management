/**
 * Schedule and roster validation rules.
 */
...
function validateStudent(student, data) {
    ...
    if (student.facilityId && !DataModel.findFacilityById(data, student.facilityId)) {
      warnings.push('Clinical site is not assigned to a known facility');
    }
    if (ClinicalSites) {
      var allowed = ClinicalSites.getGroupFacilities(data, student.clinicalGroup);
      student.schedule.forEach(function (cell, wi) {
        if (!cell || cell.inactive) return;
        if (!(cell.clinical || cell.makeupClinical)) return;
        var fac = ClinicalSites.getStudentFacilityAtWeek(data, student, wi);
        if (!fac) {
        ...
        var ok = allowed.some(function (id) {
          return DataModel.sameFacilitySite(data, id, fac);
        });
        if (!ok) {
          warnings.push('Week ' + (wi + 1) + ' clinical site not in group allowed list');
        }
        if (ClinicalSites.isWeekGapForGroup(data, student.clinicalGroup, wi)) {
  ...
  function validateSimSessions(data) {
    var cfg = data.config;
    var caps = Scheduler.getSimCaps(cfg);
    var violations = [];
    for (var w = 0; w < 18; w++) {
      if (CalendarEngine.isWeekInactive(data, w)) continue;
      DataModel.getSimDays(cfg).forEach(function (day) {
        var list = Scheduler.getDaySimStudents(data, w, day);
  ...
  function validateClinicalSessions(data) {
    var cfg = data.config;
    var caps = Scheduler.getClinicalCaps(cfg);
    var violations = [];
    DataModel.getClinicalGroups(cfg).forEach(function (group) {
      var day = DataModel.getClinicalDayForGroup(group, cfg);
      for (var w = 0; w < 18; w++) {
        if (CalendarEngine.isWeekInactive(data, w)) continue;
        var list = Scheduler.getClinicalGroupSessionStudents(data, w, group, day);
  ...
  function validateNoDoubleBooking(data) {
    ...
    data.students.forEach(function (s) {
      s.schedule.forEach(function (cell, wi) {
        if (Scheduler.weekHasDoubleBooking(cell, s, cfg)) {
          violations.push({
            studentId: s.id,
            studentName: s.name,
            week: wi + 1,
  ...
  function validateOrientationConflicts(data) {
    if (!Orientation || !Orientation.findOrientationConflicts) return [];
    return Orientation.findOrientationConflicts(data);
  }
  ...
  function validateProgramSimWeeks(data) {
    var calendar = data._simCalendar || Scheduler.buildProgramSimCalendar(data, data.config);
    var violations = [];
    for (var w = 0; w < 18; w++) {
      if (CalendarEngine.isWeekInactive(data, w)) continue;
...
export {
  validateStudent,
  validateSimSessions,
  validateClinicalSessions,
  validateNoDoubleBooking,
  validateOrientationConflicts,
  validateSimClinicalConflicts,
  validateSimGroupExceptions,
  validateSimChronologicalOrder,
  validateProgramSimWeeks,
  validateStudentSimParticipation,
  validateSimBlockNoRepeat,
  validateGroups,
  validateAll,
  statusBadge
};
