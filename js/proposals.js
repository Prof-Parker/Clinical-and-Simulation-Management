...
/**
 * Setup and clinical-site proposal workflow.
 * Spec: docs/Design Docs/User_roles_design.md § Multi-user workflow
 */
App.Proposals = (function () {
  ...
  function isStale(proposal, semester) {
    ...
    semester = semester || (typeof App.getData === 'function' ? App.getData() : null);
  ...
  function afterApprove(semester, prop) {
    ...
    var path = prop.path || '';
    if (path.indexOf('config') === 0 && App.DataModel) {
    ...
    if (path.indexOf('calendar') === 0 || path === 'holidays' ||
      ...
      if (App.CalendarEngine) App.CalendarEngine.rebuildWeeks(semester);
    }
    if (path.indexOf('facilities') === 0 || path === 'students' ||
        path.indexOf('students.') === 0) {
      if (App.DataModel) {
        App.DataModel.normalizeFacilities(semester);
        App.DataModel.syncSemesterForConfig(semester);
      }
    }
    if (App.ClinicalSites && path.indexOf('config') === 0) {
      App.ClinicalSites.applyPrimarySitesToStudents(semester);
    }
  }
  ...
  return {
    ...
    getPendingAll: getPendingAll,
    isStale: isStale,
    getValueAtPath: getValueAtPath,
    setValueAtPath: setValueAtPath,
    diffValue: diffValue,
    diffObjectTree: diffObjectTree,
    diffConfig: diffConfig,
    diffArrayById: diffArrayById,
    diffStudents: diffStudents,
    diffSetup: diffSetup,
    upsertProposal: upsertProposal,
    submitConfigProposals: submitConfigProposals,
    submitSetupProposals: submitSetupProposals,
    afterApprove: afterApprove,
    approve: approve,
    deny: deny,
    clearProposal: clearProposal,
    mergeProposalLists: mergeProposalLists,
    formatProposalLabel: formatProposalLabel
  };
})();
