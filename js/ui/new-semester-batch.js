...
App.UI.NewSemesterBatch = (function () {
  var wizardState = null;

  function openWizard() {
    if (!App.Permissions.guard('semester.batchCreate')) return;
    wizardState = {
      season: 'fall',
  ...
  function batchDraftSemester() {
    ...
    App.DataModel.applySemesterSeasonYear(sem, wizardState.season, wizardState.year);
  ...
  function renderBatchHolidays() {
    if (!App.UI.Setup || !App.UI.Setup.renderHolidays) return;
    syncBatchFormToState();
    var draft = batchDraftSemester();
    App.UI.Setup.renderHolidays(draft, 'batchHolidays');
    App.UI.Setup.bindHolidayEditor('batchHolidays', {
      getData: function () {
        syncBatchFormToState();
        var d = batchDraftSemester();
        App.UI.Setup.collectHolidaysFromDom(d, 'batchHolidays');
  ...
  function collectBatchHolidaysFromState() {
    syncBatchFormToState();
    var draft = batchDraftSemester();
    App.UI.Setup.collectHolidaysFromDom(draft, 'batchHolidays');
  ...
  function createFiles() {
    ...
    if (!selected.length) {
      App.UI.showAlert('New semester', 'Select at least one course.');
      return;
    }
    if (App.Storage.supportsDirectoryPicker()) {
      window.showDirectoryPicker().then(function (dir) {
        return saveAllToDirectory(dir, selected);
      }).then(function () {
        App.UI.showAlert('Created', 'Created ' + selected.length + ' semester file(s).');
        closeWizard();
      }).catch(function () {});
      return;
    }
  ...
  function saveAllToDirectory(dirHandle, courseIds) {
    ...
    courseIds.forEach(function (courseId) {
      chain = chain.then(function () {
        var token = App.Storage.semesterFileTokenFromMeta(wizardState.season, wizardState.year, courseId);
        var fileRoot = buildSemesterFileRoot(courseId);
        return dirHandle.getFileHandle(token + '.json', { create: true }).then(function (fh) {
          return App.Storage.writeFileRootToHandle(fh, fileRoot);
        });
      });
    });
    return chain;
  ...
  function saveSequential(courseIds, index) {
    if (index >= courseIds.length) {
      App.UI.showAlert('Created', 'Created ' + courseIds.length + ' semester file(s).');
      closeWizard();
      return;
    }
    ...
    window.showSaveFilePicker({
      ...
      return App.Storage.writeFileRootToHandle(handle, fileRoot);
    }).then(function () {
  ...
  return { init: init, openWizard: openWizard };
})();
