...
/**
 * Audit PDF export (spec: docs/AUDIT_TRACKING_IMPLEMENTATION.md §7).
 * Renders a hidden print DOM styled by css/audit-print.css and calls
 * window.print(); the browser's "Save as PDF" produces the artifact.
 */
App.AuditExport = (function () {
  ...
  function facilityForRow(semester, facilityId) {
    var fac = App.DataModel.findFacilityById(semester, facilityId);
  ...
  function buildRequirementsSummary(semester) {
    var validation = App.Validator.validateAll(semester);
    return (semester.students || []).slice().sort(function (a, b) {
      return a.name.localeCompare(b.name);
    }).map(function (s) {
      var stats = App.DataModel.countStats(s);
  ...
  function exportAuditPdf(semester, adminName) {
    ...
    return App.AuditSnapshot.computeHash(semester).then(function (hash) {
      ...
      if (App.Audit.getPhase(semester) === 'makeup_review') {
        App.Audit.setPhase(semester, 'audit_exported');
      }
      App.notifyChange();
  ...
  function promptAndExport(semester, done) {
    ...
    App.UI.showDialog('Export audit PDF', body, function () {
      ...
      exportAuditPdf(semester, adminName).then(function (result) {
        App.UI.showAlert('Audit PDF exported',
          'Save the PDF as:\n\n' + result.fileName +
        ...
        App.UI.showAlert('Export failed', String(err && err.message || err));
      });
    });
  }

  return {
    AUDIT_APP_VERSION: AUDIT_APP_VERSION,
    buildMakeupLogRows: buildMakeupLogRows,
    buildRequirementsSummary: buildRequirementsSummary,
    suggestedPdfName: suggestedPdfName,
